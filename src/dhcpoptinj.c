...
static int  inspectPacket(struct nfq_q_handle *queue,
                          struct nfgenmsg *    pktInfo,
                          struct nfq_data *    pktData,
                          void *               userData);
...
static enum MangleResult manglePacket(const uint8_t *origData,
                                      size_t         origDataSize,
                                      uint8_t **     newData,
                                      size_t *       newDataSize);
static enum MangleResult mangleOptions(const uint8_t *origData,
                                       size_t         origDataSize,
                                       uint8_t *      newData,
                                       size_t *       newDataSize);
...
int main(int argc, char *argv[])
{
   ...
   struct nfq_q_handle *queue = nfq_create_queue(nfq, config->queue, &inspectPacket, NULL);
   if (!queue)
   {
   ...
   if (config->debug)
      logMessage(LOG_DEBUG,
                 "Initialisation completed. Waiting for packets to "
   ...
   if (signalCaught)
   {
      const char *signalName = signalCaught == SIGINT ?
                                   "SIGINT" :
                                   signalCaught == SIGTERM ? "SIGTERM" : signalCaught == SIGHUP ? "SIGHUP" : "??";
...
static int inspectPacket(struct nfq_q_handle *queue, struct nfgenmsg *pktInfo, struct nfq_data *pktData, void *userData)
{
   (void)pktInfo;
   (void)userData;
   ...
   enum MangleResult result          = manglePacket(packet, (size_t)size, &mangledData, &mangledDataSize);
   ...
   int res = nfq_set_verdict(queue, ntohl(metaHeader->packet_id), NF_ACCEPT, mangledDataSize, mangledData);
   free(mangledData);
   return res;
}
                                      ...
                                      size_t *       newDataSize)
{
   ...
   *newData = malloc(*newDataSize);
   ...
   enum MangleResult result = mangleOptions(origData, origDataSize, *newData, newDataSize);
   if (result != Mangle_OK)
   {
      free(*newData);
      return result;
   ...
   *newDataSize = ipHdrSize + udpHdrSize + newPayloadSize;
   ...
   if (padding && config->debug)
      logMessage(LOG_DEBUG,
                 "Padding with %zu byte(s) to meet minimal BOOTP payload "
                 "size\n",
                 padding);
                                       ...
                                       size_t *       newDataSize)
{
   ...
   while (origOffset < origDataSize)
   {
      ...
      size_t                   optSize =
          option->code == DHCPOPT_PAD || option->code == DHCPOPT_END ? 1 : sizeof(struct DHCPOption) + option->length;
...
__attribute__((__format__(__printf__, 2, 0))) static void logMessage(int priority, const char *format, ...)
{
...
static void debugLogOptions(void)
{
   ...
   logMessage(LOG_DEBUG,
              "%zu DHCP option(s) to inject (with a total of %zu bytes): ",
   ...
   for (size_t i = 0; i < config->dhcpOptCodeCount; ++i)
   {
      ...
      logMessage(LOG_DEBUG, "%u (0x%02X) (%s)%s", code, code, dhcp_optionString(code), delim);
   }
   logMessage(LOG_DEBUG, "Existing options will be %s\n", config->removeExistOpt ? "removed" : "left in place");
}
...
static void debugLogPacketHeader(const uint8_t *data, size_t size)
{
   ...
   logMessage(LOG_DEBUG,
              "Inspecting %zu-byte DHCP packet from "
...
static void debugLogOptionFound(const struct DHCPOption *option)
{
   if (option->code == DHCPOPT_PAD)
      ...
      logMessage(LOG_DEBUG, "Found END option %s\n", config->dhcpOptCodeCount ? "(removing)" : "(copying)");
   else if (option->code == DHCPOPT_TYPE && option->length == 1)
      logMessage(LOG_DEBUG,
                 "Found option % 3hhd (0x%02hhX) (DHCP message type)        %s",
                 option->code,
                 option->code,
                 dhcp_msgTypeString(option->data[0]));
   else
...
static void debugLogOption(const char *action, const struct DHCPOption *option)
{
   ...
   logMessage(LOG_DEBUG,
              "%s option % 3hhd (0x%02hhX) (%s)%*s with % 3d-byte payload %s",
              action,
              option->code,
              option->code,
              optName,
